...
const TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      supporting_documents: { type: ['string', 'null'], description: 'Whether they have supporting docs if needed (proof of NI, proof of address), verbatim, else null.' },
      driving_licence: { type: ['boolean', 'null'], description: 'Do they drive / hold a licence.' },
      own_transport: { type: ['boolean', 'null'], description: 'Do they have access to their own vehicle.' },
      ...
      competency_standards: { type: ['string', 'null'], description: 'Their answer to the maintaining-standards competency question, summarised. null if they gave none.' },
      competency_communication: { type: ['string', 'null'], description: 'Their answer to the communication competency question, summarised. null if they gave none.' },
      previous_experience: { type: ['string', 'null'], description: 'Summary of their previous cleaning/housekeeping experience (where, when, responsibilities); or, if none, any relevant transferable experience they gave. null if not covered.' },
      availability_changes: { type: ['string', 'null'], description: 'Any upcoming holidays, commitments or extended periods where their availability will change, verbatim. null if none/not covered.' },
      interview_availability: { type: ['string', 'null'], description: 'When the candidate said they could attend a next-stage hiring-manager interview if shortlisted (days/times, plus any flexibility), verbatim. null if not covered.' },
      summary: { type: 'string', description: '1–2 sentence neutral recruiter summary, third person. Only facts stated on the call. If interview_availability was given, append it as a final sentence, e.g. "Free for a next-stage interview: <days/times>."' },
    ...
    const { error: srErr } = await db.from('fc_candidate_structured_responses').upsert(
      {
        ...
        supporting_documents: x.supporting_documents ?? null,
        ...
        previous_experience: x.previous_experience ?? null,
        available_days: (x.available_days as string[]) ?? [],
        availability_changes: x.availability_changes ?? null,
        ...
        interview_availability: x.interview_availability ?? null,
        competency_standards: x.competency_standards ?? null,
        competency_communication: x.competency_communication ?? null,
      },
